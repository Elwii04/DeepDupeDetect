...
DINOV3_MODELS = {
    'vitl': {'name': 'facebook/dinov3-vitl16-pretrain-lvd1689m', 'feature_dim': 1024, 'batch_size': 200, 'type': 'dinov3'}
}
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.model_type == 'dinov3':
                # For DINOv3, return PIL image directly - preprocessing happens in collate_fn
                return img, abs_path
            else:
...
class DINOv3CollateFn:
    """Picklable collate function for DINOv3 that works with multiprocessing."""
    ...
    @property
    def processor(self):
        # Lazy load processor in each worker process
    ...
    def __call__(self, batch):
        ...
        # This is the correct way - processor handles the batch
...
class ImageDeduplicator:
    ...
    def _extract_features(self, image_batch):
        ...
        with torch.no_grad():
            ...
            if self.model_config['type'] == 'dinov3':
                ...
                # Official DINOv3 usage: pooler_output as global embedding
                features = outputs.pooler_output
                if features is None:
                    # Fallback: mean over tokens when pooler isn't provided
            ...
            if self.save_both_embeddings:
                ...
                last_hidden_features = last_hidden_features / last_hidden_features.norm(dim=-1, keepdim=True).clamp_min(1e-12)
                final_features = final_features / final_features.norm(dim=-1, keepdim=True).clamp_min(1e-12)
                
                # Ensure stable, float32 embeddings before saving
                last_hidden_features = torch.nan_to_num(last_hidden_features, nan=0.0, posinf=0.0, neginf=0.0).float()
                final_features = torch.nan_to_num(final_features, nan=0.0, posinf=0.0, neginf=0.0).float()
            ...
            if not self.save_both_embeddings:
                ...
                features = torch.nan_to_num(features, nan=0.0, posinf=0.0, neginf=0.0).float()
                return features
    ...
    def _check_database_compatibility(self):
        ...
        if not self.save_both_embeddings:
            ...
            sample_vec = np.frombuffer(sample_hash, dtype=np.float32)
            stored_dim = len(sample_vec)
            
            # Detect obviously bad stored embeddings (e.g., all NaNs or zeros)
            if np.isnan(sample_vec).all() or np.all(sample_vec == 0):
                print(f"\n⚠️  WARNING: Existing database hashes appear invalid (all NaN/zero).")
                user_choice = input("Do you want to re-hash all images to fix the database? (y/n): ").lower().strip()
                if user_choice == 'y':
                    self.cursor.execute("DELETE FROM images")
                    self.conn.commit()
                    ...
                    print("Continuing with existing (possibly invalid) hashes.")
                    return False
    ...
    def generate_hashes(self, rescan_all):
        ...
        for image_batch, paths in tqdm(data_loader, desc="Hashing Images"):
            ...
            if self.model_config['type'] == 'dinov3':
                image_batch = image_batch.to(self.device, dtype=torch.float32, non_blocking=True)
            else:
            ...
            if self.save_both_embeddings:
                ...
                cpu_last_hidden = last_hidden_features.float().cpu().numpy()
                cpu_final = final_features.float().cpu().numpy()
                ...
                cpu_features = features.float().cpu().numpy()
    ...
    def find_and_remove_duplicates(self, custom_threshold=None):
        ...
        def _load_chunk_to_gpu(offset, size):
            ...
            tensor = torch.from_numpy(hashes_np).to(self.device, dtype=torch.float32, non_blocking=True)
            tensor = torch.nn.functional.normalize(tensor, dim=1, eps=1e-12)
            return tensor.half()
